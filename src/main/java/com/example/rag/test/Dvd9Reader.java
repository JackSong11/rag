...
/**
 * 解析 /Users/songzhiquan1/IdeaProjects/rag/data/index9/_9_Lucene90_0.dvd 文件内容
 * .dvd 文件是 Lucene90 DocValues 的数据文件 (DocValues Data)。
 * 它存储了 DocValues 字段的实际数据，包括 Numeric、Binary、Sorted、
 * SortedNumeric、SortedSet 等类型的值。
 *
 * 解析方式：
 * 1. 底层：解析 Codec Header / Footer 及原始数据的 Hex Dump
 * 2. 高层：通过 Lucene API 读取该 segment 中 DocValues 字段的实际数据
 *
 * 将解析结果保存到 data/output/9/_9_Lucene90_0_dvd_content.txt
 */
public class Dvd9Reader {

    private static final String INDEX_PATH = "/Users/songzhiquan1/IdeaProjects/rag/data/index9";
    private static final String FILE_NAME = "_9_Lucene90_0.dvd";
    private static final String OUTPUT_PATH = "/Users/songzhiquan1/IdeaProjects/rag/data/output/9/_9_Lucene90_0_dvd_content.txt";
    ...
    public static void main(String[] args) {
        try {
            ...
            try (PrintWriter writer = new PrintWriter(new FileWriter(OUTPUT_PATH))) {
                ...
                writer.println("==========================================================");
                writer.println(" Lucene _9_Lucene90_0.dvd 文件解析 (DocValues Data)");
                ...
                // ====== Part 2: 通过 Lucene API 读取 DocValues 实际数据 ======
    ...
    /**
     * Part 1: 底层解析 .dvd 文件的 Codec Header、Footer 和原始 Hex Dump
     */
    ...
    /**
     * Part 2: 通过 Lucene DirectoryReader API 解析 DocValues 实际数据内容
     * .dvd 文件存储了 DocValues 的实际数据，通过 API 可以读取每个文档的 DocValues 值
     */
    private static void parseWithLuceneAPI(PrintWriter writer) throws Exception {
        writer.println("==========================================================");
        writer.println(" 通过 Lucene API 解析 DocValues 实际数据");
        writer.println(" (对应 _9_Lucene90_0.dvd 文件中存储的 DocValues 数据)");
        writer.println("==========================================================");
        writer.println();
    ...
    private static void parseNumericDocValues(LeafReader reader, String field, PrintWriter writer) throws Exception {
        ...
        writer.println("  --- Numeric DocValues 数据 ---");
        int count = 0;
    ...
    private static void parseBinaryDocValues(LeafReader reader, String field, PrintWriter writer) throws Exception {
        ...
        writer.println("  --- Binary DocValues 数据 ---");
        int count = 0;
    ...
    private static void parseSortedDocValues(LeafReader reader, String field, PrintWriter writer) throws Exception {
        ...
        writer.println("  --- Sorted DocValues 数据 ---");
        writer.println("  唯一值数量: " + valueCount);
        writer.println();

        // 输出所有唯一值(ord -> value)
        writer.println("  [Ord -> Value 映射]");
        for (int ord = 0; ord < Math.min(valueCount, 100); ord++) {
            ...
            writer.printf("    ord=%d -> \"%s\"%n", ord, val.length() > 100 ? val.substring(0, 100) + "..." : val);
        }
        if (valueCount > 100) {
            writer.println("    ... (超过100个唯一值，截断显示)");
        }
        writer.println();

        // 输出 doc -> ord 映射
        writer.println("  [Doc -> Ord -> Value 映射]");
        int count = 0;
    ...
    private static void parseSortedNumericDocValues(LeafReader reader, String field, PrintWriter writer) throws Exception {
        ...
        writer.println("  --- Sorted Numeric DocValues 数据 ---");
        int count = 0;
    ...
    private static void parseSortedSetDocValues(LeafReader reader, String field, PrintWriter writer) throws Exception {
        ...
        writer.println("  --- Sorted Set DocValues 数据 ---");
        writer.println("  唯一值数量: " + valueCount);
        writer.println();

        // 输出所有唯一值
        writer.println("  [Ord -> Value 映射]");
        for (long ord = 0; ord < Math.min(valueCount, 100); ord++) {
        ...
        // 输出 doc -> ords 映射
        writer.println("  [Doc -> Ords -> Values 映射]");
        int count = 0;
        while (ssdv.nextDoc() != SortedSetDocValues.NO_MORE_DOCS) {
            ...
            while ((ord = ssdv.nextOrd()) != SortedSetDocValues.NO_MORE_ORDS) {
                ...
                int length = ssdv.lookupOrd(ord).length;
                String val = new String(bytes, offset, length, java.nio.charset.StandardCharsets.UTF_8);
                ords.append("ord=" + ord + "/\"" + (val.length() > 40 ? val.substring(0, 40) + "..." : val) + "\"");
            }
