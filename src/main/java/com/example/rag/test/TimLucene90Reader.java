...
/**
 * 解析 /Users/songzhiquan1/IdeaProjects/rag/data/index9/_9_Lucene90_0.tim 文件内容
 * .tim 文件是 Lucene 的 Term Dictionary 文件，存储了所有 term 的字典数据。
 * 这个文件使用的是 Lucene90PostingsFormat 编解码器。
 *
 * 解析方式：
 ...
 *
 * 将解析结果保存到 data/output/9/_9_Lucene90_0_tim_content.txt
 */
public class TimLucene90Reader {

    private static final String INDEX_PATH = "/Users/songzhiquan1/IdeaProjects/rag/data/index9";
    private static final String FILE_NAME = "_9_Lucene90_0.tim";
    private static final String OUTPUT_PATH = "/Users/songzhiquan1/IdeaProjects/rag/data/output/9/_9_Lucene90_0_tim_content.txt";
    ...
    public static void main(String[] args) {
        try {
            ...
            try (PrintWriter writer = new PrintWriter(new FileWriter(OUTPUT_PATH))) {
                ...
                writer.println("==========================================================");
                writer.println(" Lucene _9_Lucene90_0.tim 文件解析 (Term Dictionary)");
                writer.println(" 编解码器: Lucene90PostingsFormat");
                writer.println("==========================================================");
     ...
     * _9_Lucene90_0.tim 对应的是 Lucene90PostingsFormat 写入的字段
     * 这些字段与 _9_0.tim（BlockTreeTermsDict）中的字段是不同的
     */
    private static void parseWithLuceneAPI(PrintWriter writer) throws Exception {
        ...
        writer.println(" (对应 _9_Lucene90_0.tim 文件中存储的 term 数据)");
        writer.println("==========================================================");
        writer.println();
        ...
        try (FSDirectory dir = FSDirectory.open(Paths.get(INDEX_PATH));
            ...
            for (LeafReaderContext leafContext : reader.leaves()) {
                ...
                for (FieldInfo fieldInfo : fieldInfos) {
                    ...
                    // 通过检查 term dictionary 的实现类来区分不同 PostingsFormat 的字段
                    // Lucene90PostingsFormat 对应的 tim 文件后缀是 "Lucene90"
                    // 我们输出所有字段的 term 信息（Lucene API 会自动路由到正确的文件）
                    writer.println("----------------------------------------------------------");
